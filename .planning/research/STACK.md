...
**Domain:** AI Social Media Manager & WhatsApp CRM for clinics (single-tenant, VPS-hosted)
**Researched:** 2026-03-27
**Confidence:** MEDIUM — external search tools unavailable; based on training data through August 2025 plus ecosystem knowledge. All version recommendations should be verified against official release pages before pinning in docker-compose.
...
docker pull n8nio/n8n:latest             # pin to specific tag in production
docker pull atendai/evolution-api:v2     # pin to specific v2.x tag
...
**If VPS has 4 GB RAM (entry-level Hostinger plan):**
...
**If VPS has 8 GB RAM (recommended minimum for Ollama):**
...
**If WhatsApp session becomes unstable (common with Evolution API):**
...
**If social media publishing is the first priority (vs. WhatsApp CRM):**
...
**If chatbot quality is below the 80% FAQ resolution target:**
...
**Recommendation: Start with OpenAI gpt-4o-mini, plan migration to Ollama if VPS has 8+ GB RAM.**

The 80% FAQ resolution criterion in the success criteria is harder to hit with local 3B models than it looks. FAQ chatbots fail not due to model size but due to poor system prompt design and lack of clinic-specific context. OpenAI gpt-4o-mini + a well-designed system prompt with clinic FAQ content will reliably exceed 80% from day one. Ollama with llama3.2:3b will require tuning effort. Since the VPS RAM situation is unknown until provisioned, default to OpenAI to de-risk the MVP milestone, then migrate to Ollama in Phase 2 if cost becomes relevant.

**Confidence: MEDIUM** — based on general LLM evaluation patterns; specific Hostinger VPS hardware specs are unknown.

### Decision 2: Streamlit vs. n8n Forms for Admin UI

**Recommendation: Streamlit for the full admin UI.**

n8n's built-in form trigger is useful for one-off manual triggers (e.g., "send campaign now" button) but cannot handle bulk CSV import, patient segmentation logic, or a data grid showing patient records. Streamlit is the fastest path to a functional Python-based admin panel that connects directly to Postgres. The full admin UI (patient CRUD, campaign launch, metrics dashboard) belongs in Streamlit. Simple single-action triggers can remain as n8n webhook URLs called by Streamlit.

**Confidence: HIGH** — well-established pattern for internal Python admin tools.

### Decision 3: Evolution API Session Stability

**Recommendation: Treat Evolution API as a third-party dependency with a managed failure mode.**

Evolution API depends on the Baileys library's reverse-engineered WhatsApp Web protocol. WhatsApp periodically changes its Web protocol, causing Baileys (and thus Evolution API) to break until a patch is released. This is a known and accepted tradeoff (per PROJECT.md). Mitigation: (1) pin to a stable release tag, (2) build an n8n monitoring workflow that checks `/instance/connectionState` every 5 minutes and sends an alert if disconnected, (3) document the reconnection procedure (re-scan QR code) so the clinic operator can self-recover.

**Confidence: HIGH** — this instability pattern is well-documented in the Evolution API community.
...
---
*Stack research for: AI Social Media Manager & WhatsApp CRM — single-tenant VPS deployment*
*Researched: 2026-03-27*
